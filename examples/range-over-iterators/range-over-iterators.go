...
// [^iterators]: iterators – повторители, итератори – функции, които се ползват с `range`
...
// който връщаше резен с всички членове на списъка. С
...
func main() {
	...
	// Пакетите от стандартната библотека сега вече предоставят помощни
	// функции, връщащи повторители. Например функцията `strings.SplitSeq`
	// извършва повтарящо се действие върху частите на резен от байтове, без
	// предварително да изгражда резен, а връща повторител за еднократна употреба.
	for part := range strings.SplitSeq("Го-в-примери", "-") {
		fmt.Printf("част: %s\n", part)
	}

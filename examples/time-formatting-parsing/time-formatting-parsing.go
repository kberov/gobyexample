...
func main() {
	...
	// изглед (формат).
	t1, e := time.Parse(time.RFC3339, "2026-03-10T21:26:41+02:00")
	p(t1)
	...
	// Методът `Parse` ще върне грешка за развален вход и ще обясни
	// каква е причината за грешката. (В този случай денят от
	// седмицата няма как да бъде разпознат в подаденото време
	// "8:41PM")
